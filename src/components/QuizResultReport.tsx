      ...
      <div className="space-y-5">
        ...
        <div className="p-3 bg-background border rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="font-bold text-primary">삶의 에너지 상태</div>
            <span className={`text-xs px-2 py-1 rounded ${
              REPORT_DATA.energy[energy]?.priority === '긴급' ? 'bg-red-100 text-red-800' :
              REPORT_DATA.energy[energy]?.priority === '높음' ? 'bg-orange-100 text-orange-800' :
              REPORT_DATA.energy[energy]?.priority === '보통' ? 'bg-yellow-100 text-yellow-800' :
          ...
          <div className="text-sm bg-blue-50 p-2 rounded">
            <strong>💊 처방:</strong> {REPORT_DATA.energy[energy]?.advice}
          </div>
        </div>
        ...
        <div className="p-3 bg-background border rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-lg">🤝</span>
            <div className="font-bold text-primary">정서적 지지 환경</div>
            <span className={`text-xs px-2 py-1 rounded ${
              REPORT_DATA.support[support]?.priority === '긴급' ? 'bg-red-100 text-red-800' :
              REPORT_DATA.support[support]?.priority === '높음' ? 'bg-orange-100 text-orange-800' :
              REPORT_DATA.support[support]?.priority === '보통' ? 'bg-yellow-100 text-yellow-800' :
              'bg-green-100 text-green-800'
            }`}>
          ...
          <div className="text-sm bg-blue-50 p-2 rounded">
            <strong>💊 처방:</strong> {REPORT_DATA.support[support]?.advice}
          </div>
        </div>
        ...
        <div className="p-3 bg-background border rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-lg">⏰</span>
            <div className="font-bold text-primary">'나'만의 시간 확보</div>
            <span className={`text-xs px-2 py-1 rounded ${
              REPORT_DATA.time[time]?.priority === '긴급' ? 'bg-red-100 text-red-800' :
              REPORT_DATA.time[time]?.priority === '높음' ? 'bg-orange-100 text-orange-800' :
              REPORT_DATA.time[time]?.priority === '보통' ? 'bg-yellow-100 text-yellow-800' :
              'bg-green-100 text-green-800'
            }`}>
          ...
          <div className="text-sm bg-blue-50 p-2 rounded">
            <strong>💊 처방:</strong> {REPORT_DATA.time[time]?.advice}
          </div>
        </div>
        ...
        <div className="p-3 bg-background border rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-lg">🎯</span>
            <div className="font-bold text-primary">맞춤형 실천 제안</div>
          </div>
          <div className="mb-2">
            {Array.isArray(needs) && needs.length > 0 ? (
              needs.map((needIndex: number) => (
                <span key={needIndex} className="inline-block bg-accent px-2 py-1 rounded mr-1 mb-1 text-xs">
          ...
          <ul className="text-sm space-y-2">
            {Array.isArray(needs) && needs.length > 0 ? needs.map((needIndex: number) => (
              <li key={needIndex} className="bg-blue-50 p-2 rounded">
                <strong>💊 처방:</strong> {REPORT_DATA.needs[needIndex]}
              </li>
            )) : (
